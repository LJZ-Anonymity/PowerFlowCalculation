...
/*
 * 本文件参考“代码示例/Python--Newton-raphson-method-flow-calculation-main/转译后C语言代码”
 * 中的结构，把牛顿-拉夫逊潮流计算的核心逻辑整合到一个 C 源文件中，
 * 便于课程设计直接使用。
 *
 * 当前版本：
 *  - 使用直角坐标牛顿-拉夫逊法（e,f 变量）；
 *  - 在 main 中内置一个 5 节点测试系统（类似 case5）；
 *  - 形成节点导纳矩阵 G+jB；
 *  - 迭代求解各节点电压，收敛后在屏幕上输出结果。
 *
 * 后续你可以根据课程要求：
 *  - 把 main 里的线路、节点数据改为从文件读取；
 *  - 增加潮流结果的详细输出（支路功率、网损等）。
 */
    ...
    while (fgets(buf, sizeof(buf), fp)) {
            ...
            *out_node_count = (size_t)nNode;
            for (i = 0; i < nNode; ++i) {
                ...
                if (!fgets(buf, sizeof(buf), fp) ||
                    ...
                    sscanf_s(buf, "%d %d %lf %lf %lf %lf",
                             &k_idx, &bus_i, &PG, &QG, &PL, &QL) != 6) {
        ...
        if (state == 6) {
            ...
            for (i = 0; i < nPV; ++i) {
                ...
                if (!fgets(buf, sizeof(buf), fp) ||
                    !strip_comment_and_trim(buf) ||
                    sscanf_s(buf, "%d %d %lf %lf %lf",
                             &idx, &bus_i, &Vi, &Qmin, &Qmax) != 5) {
...
static int locate_slack(const NetworkInfo *info)
{
    int i;
    for (i = 0; i < info->order; ++i) {
        if (info->nodes[i].type == NODE_SLACK) {
            return i;
        }
    }
    return -1;
                                  ...
                                  double *Q_calc)
{
    ...
    if (!info || !P_calc || !Q_calc || !info->e || !info->f) {
        return;
    }
...
static void fill_row_pair(NLIteration *ctx, int row_idx,
                          double *row1, double *row2)
{
    ...
    {
        double Gii = mat_get(info->G, info->order, node_idx, node_idx);
        double Bii = mat_get(info->B, info->order, node_idx, node_idx);
        double J_ii = N_ii - Bii * f_i - Gii * e_i;
        double L_ii = -H_ii + Gii * f_i - Bii * e_i;
...
static void solve_linear_system(const double *A, const double *b,
                                double *x, int n)
{
    int i, j, col;
    ...
    double *aug = (double *)calloc((size_t)n * (size_t)(n + 1),
                                   sizeof(double));
...
int main(void)
{
    ...
    const char *input_path = "InputData/case5.txt";
